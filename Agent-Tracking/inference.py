...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for x in range(0, X_RANGE) :
        for y in range(0, Y_RANGE) :
            allGridPosition.append((x, y))
            
    variableDomainsDict[PAC] = allGridPosition
    variableDomainsDict[GHOST0] = allGridPosition
    variableDomainsDict[GHOST1] = allGridPosition

    possibleManhattanDistance = []
    
    for d in range(-MAX_NOISE, X_RANGE+Y_RANGE+MAX_NOISE - 1):
        if d >= 0:
            possibleManhattanDistance.append(d)
            
    variableDomainsDict[OBS0] = possibleManhattanDistance
    variableDomainsDict[OBS1] = possibleManhattanDistance
    #raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # Initialize factors with evidence and query variables
        factorList = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # Iterate through the elimination order
        for var in eliminationOrder:
        # Check if the variable is in any of the factors
            factorList, joinedFactor = joinFactorsByVariable(factorList, var)
            factorList.append(joinedFactor)
            ...
            if len(joinedFactor.unconditionedVariables()) > 1:
                ...
                factorList.remove(joinedFactor)
                factorList.append(eliminatedFactor)
            else:
                factorList.remove(joinedFactor)
        # Join remaining factors
        finalFactor = joinFactors(factorList)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        trueDis = manhattanDistance(pacmanPosition, ghostPosition)
        if ghostPosition == jailPosition:
            ...
            if noisyDistance != None:
                return busters.getObservationProbability(noisyDistance, trueDis)
            else:
                return 0
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        #Equation in Note 19 Page 5
        for pos in self.allPositions:
            #The conditional probability of P(pos(i+1)|pos(i))
            newPosDist = self.getPositionDistribution(gameState, pos)
            newPos = newPosDist.keys()
            #Sums over all the new positions you could go to
            for p in newPos:
                cond_prob = newPosDist[p]
                updatedBelief = cond_prob * self.beliefs[pos]
                newBelief[p] += updatedBelief
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        # print(self.numParticles) -> 5000 particles for 9 positions
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            ...
            #Calculating the weight of a particle
            weight = self.getObservationProb(observation, 
                    gameState.getPacmanPosition(), particle, self.getJailPosition())
            
            #Calculating the total weight for each state
            belief[particle] += weight
        
        #If the sum of all weights across all states is 0, reinitialize
